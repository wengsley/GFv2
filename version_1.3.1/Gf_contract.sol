...
contract GFTEST3 is IBEP20, Auth {
    ...
    string private constant _symbol = "GFTEST3";
    ...
    function getSwapFees() internal {
        tempLiquidityFee = liquidityFee.add(sellLiquidityFee).div(2); // (1+0)/2 = 0.5
        tempMarketingFee = marketingFee.add(sellMarketingFee).div(2);
        tempRewardFee = rewardFee.add(sellRewardFee).div(2); // (1+1)/2 = 1
        tempTotalFee = totalFee.add(totalSellFee).div(2); // (3+3)/2 = 3
    }
    ...
    function swapBack() internal swapping {
        ...
        // (balance * 0.5 /(3-1))/2 = (balance * 0.5 /2 ) /2 = ((100 * 0.5)/2)/2 = 12.5
        ...
        uint256 amountBNB = address(this).balance.sub(balanceBefore); //balance before t
